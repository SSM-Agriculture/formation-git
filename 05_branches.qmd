...
## Les relectures ou revues de code avec Gitlab (1/3) ![](img/icone_gitlab.png)

Les fusions de branches sont aussi l'occasion d'effectuer **des tests** ou **des revues de code** entre les membres de l'équipe projet.  

![](img/gitlab_merge_request_ajout_commentaire.png){fig-align="center"} 

## Les relectures ou revues de code avec Gitlab (2/3) ![](img/icone_gitlab.png)

![](img/gitlab_merge_request_finir_review.png){fig-align="center"} 

## Les relectures ou revues de code avec Gitlab (3/3) ![](img/icone_gitlab.png)

Une fois la revue de code effectuée, appuyer sur le bouton "Merge" pour valider la fusion :  

![](img/gitlab_merge_request_bouton_merge.png){fig-align="center"} 

::: callout-note
- Voir [cette page](https://docs.gitlab.com/ee/user/project/merge_requests/reviews/) pour la documentation complète officielle sur les merge request reviews.
:::

## Stratégie de fusion : le `fast-forward-merge`

Il existe 2 principales stratégies lors des fusions de branches dans Git.   
La première qui correspond aux cas simples et rapide (`fast`) est le `fast-forward-merge`.  

Le `fast-forward-merge` peut se faire à partir du moment où il existe **un lien linéaire** entre la branche de travail et la branche-cible.  
C'est le cas lorsque vous ajoutez plusieurs commits sur la branche de travail pendant que la branche cible reste inchangée.  
