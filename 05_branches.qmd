...
Sans rentrer dans le détail, il existe 2 principales stratégeries de merge dans Git :  
...
Les conflits surviennent généralement lorsque [**deux personnes ont modifié les mêmes lignes dans un fichier**]{.souligneRouge}, ou si un développeur a supprimé un fichier alors qu'un autre développeur le modifiait.  
...
## Gérer les conflits (1/3)

Pour repérer les conflifts, que ce soit avec le terminal ou sur Gitlab, on est vite avertis :)  
...
Et si jamais cela ne suffisait pas, on reçoit un mail pour nous dire qu'une fusion n'a pas pu avoir lieu.

## Gérer les conflits (2/3)
...
![](img/gitlab_boutons_resolution_conflits.png){fig-align="center"} 
...
Cela signifie que l'on utilise que l'arbitrage par l'équipe projet se fait directement sur l'interface Gitlab. Il s'agira ensuite de récupérer ces modifications sur le dépôt local.


## Gérer les conflits (3/3)
...
## Résoudre les conflits avec le terminal ![](img/icone_terminal.png)

[Pour résoudre le conflit, il suffit de se laisser guider par les instructions proposées par Gitlab :]{.red}

![](img/resolution_conflit_localement.png){fig-align="center"} 
...
parler du git fetch -p origin

Pour identifier les conflits pendant un merge
git log --merge
...
- Pousser la branche `bugfix-tidyverse` et la fusionner avec la branche permanente sur Gitlab.  
- Récupérer les résultats du merge dans le dépôt local.  
...
- Se positionner sur la branche permanente.  
- Fusionner la branche `bugfix-tidyverse` dans la branche permanente.  
- Puis fusionner la branche `bugfix-tidyr` dans la branche permanente.
- Résoudre le conflit
